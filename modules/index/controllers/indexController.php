...
//        Session::destroy();
    ...
    public function getNewRol(){
        if(!$this->postOk('_keypassw')){
        ...
        foreach (Session::get('sys_roles') as $value) {
            if($value['id_rol'] == $idRol){
                Session::set('sys_rol', $value['id_rol']);
                Session::set('sys_nombreRol', $value['rol']);
            }
        }
        $result = array('result'=> Obj::run()->AesCtr->en('1'));
        echo json_encode($result);
    }
